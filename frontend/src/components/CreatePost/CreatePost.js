...
import { supabase } from "../../services/supabaseClientMock.js"
...
const CreatePost = ({ closeModal }) => {
    ...
    const handleImageUpload = (e) => {
        if (e.target.files && e.target.files[0]) {
            handleImageUploadToSupabase(e.target.files[0])
        }
    }
    ...
    const handleImageUploadToSupabase = async(image)=>{
                
        let filename = `${image.name}-${Math.floor(Math.random() * 1000)}-${Date.now()}` 
        ...
        setSelectedImage(response.data.publicUrl)
    }
...
<img src={selectedImage} alt="Selected" className="w-full h-48 sm:h-64 mb-4 rounded-md object-cover" />
                        ...
                        <label className="cursor-pointer">
                            ...
                            <input type="file" className="hidden" data-testid="image-upload" onChange={handleImageUpload} />
                        </label>
                    </div>
                    )
